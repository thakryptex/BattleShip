...
//                    attacker.getPlayerField().getCell(j, k).setState(Cell.CellState.SHOOTED);
            ...
            for (int j = 0; j < cells.size(); j++) {
                player.getPlayerField().getCell(cells.get(j).getY(), cells.get(j).getX()).setState(Cell.CellState.DAMAGED);
                attacker.getEnemyTemplate().getCell(cells.get(j).getY(), cells.get(j).getX()).setState(Cell.CellState.DAMAGED);
            }
        }
        cells.clear();
